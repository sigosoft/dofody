...
class Video extends CI_Controller {
  ...
  public function token_patient($call_id)
	...
	public function talk_to_doctor($param)
	{
		...
		$arr = $this->Common->get_details('calls',array('call_id' => $param));
		if ($arr->num_rows() > 0) {
			$res = $arr->row();
			$data = $this->token_patient($param);
			$data['request'] = $res->request_id;
			$this->load->view('patient/video_consultation',$data);
		}
		else {
			redirect('patient/patients_requests');
		}
	}
