...
<html>
                  ...
                  <div class="row">
                    ...
                    <div class="col-12">
                      <form action="<?=site_url('doctor/start_consultation')?>" method="post">
                        ...
                        <button type="submit" class="btn btn-gradient btn-rounded waves-light waves-effect w-md pull-right" id="assign-doctor">Assign to doctor</button>
                      </form>
                    </div>
                    <div class="col-12">
                      <div class="col-md-6 col-md-push-3" id="accept-doctor-div" style="display:none;">
                        <table id="selected_doctor_details">

                        </table>
                        <div class="text-center" style="margin-top : 10px;">
                          <form action="<?=site_url('admin/assign_doctor')?>" method="post">
                            <input type="hidden" name="doctor_id" id="doctor_id">
                            <input type="hidden" name="request_id" id="request_id" value="<?=$req->req_id?>">
                            <button type="button" class="btn btn-success" id="choose-another-doctor">Choose another doctor</button>
  ...
  $('#assign-doctor').on('click',function(){
    $.ajax({
        url: "<?=site_url('admin/get_online_doctors')?>",
        type: 'POST',
  ...
  function doctor_selected(id)
  {
    ...
    $.ajax({
        url: "<?=site_url('admin/get_details_of_a_doctor')?>",
        type: 'POST',
        data: { id : id },
        success: function (data) {
          var array = JSON.parse(data);
          $('#selected_doctor_details').html(array.table);
          $('#doctor-select-button').css('display','none');
          $('#accept-doctor-div').css('display','block');
          $('#doctor_id').val(id);
        }
      });
  }
  $('#choose-another-doctor').on('click',function(){
    $('#doctor-select-button').css('display','block');
    $('#accept-doctor-div').css('display','none');
  });
  </script>
</html>
