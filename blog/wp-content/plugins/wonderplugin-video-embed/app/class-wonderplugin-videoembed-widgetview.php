...
class WonderPlugin_Videoembed_Widgetview {
	...
	function show_widgetfront($args, $instance) {
		...
		if ($instance['lightbox'])
		{
			...
			if ($instance['lightboxsize'])
				$content .= ' lightboxsize=1 lightboxwidth=' . $instance['lightboxwidth'] . ' lightboxheight='  . $instance['lightboxheight'];
				
			if ($instance['autoopen'])
				$content .= ' autoopen=1 autoopendelay=' . $instance['autoopendelay'];
	...
	?>
		<div class="wpve-tab-container">
			...
			<p <?php if(!class_exists('WonderPlugin_Lightbox_Plugin')) echo 'style="color:#ff0000;"'; ?>>* To play in lightbox, <a href="https://www.wonderplugin.com/wordpress-lightbox/?product=videoembed" target="_blank">WonderPlugin Lightbox</a> must be installed and activated.</p>
